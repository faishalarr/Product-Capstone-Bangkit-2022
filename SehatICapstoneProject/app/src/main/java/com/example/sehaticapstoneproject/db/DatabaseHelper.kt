...
import com.example.sehaticapstoneproject.db.DatabaseContract.ChatHistoryColumns
import com.example.sehaticapstoneproject.db.DatabaseContract.UserColumns

class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME,
    null, DATABASE_VERSION) {

    companion object {
        private const val DATABASE_NAME = "chatbotbangkit"
    ...
    override fun onUpgrade(
        ...
        db?.execSQL(
            "DROP TABLE IF EXISTS ${ChatHistoryColumns.TABLE_NAME}")
        db?.execSQL(
